...
needs to be made into a custom class in a seperate file
whole file system needs to be cleaned
this file needs to be re-tested by Joe
...
font = FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255)
...
_,f = c.read() # delete?
gray = cv2.cvtColor(f, cv2.COLOR_BGR2GRAY) # delete?
while True:
    _,f = c.read() # delete?
    gray = cv2.cvtColor(f, cv2.COLOR_BGR2GRAY) # delete?
    ...
    if len(areaContours)>0:
        if max(areaWithHull)>1000:
            ...
            cv2.putText(res,"{:0.2f}".format(ratioOfAreas), (0, res.shape[0]-80), font)

            if ratioOfAreas>0.5 and ratioOfAreas<0.65:
                cv2.putText(res,'T-Rex', (0, res.shape[0]-50), )
            elif ratioOfAreas>0.70 and ratioOfAreas<0.77:
                cv2.putText(res, 'Stegosaurus', (0, res.shape[0]-50), font)
            elif ratioOfAreas>0.78 and ratioOfAreas<0.84:
                cv2.putText(res, 'Triceratops', (0, res.shape[0]-50), font)
            elif ratioOfAreas>0.86:
                cv2.putText(res, 'Volcano', (0, res.shape[0]-50), font)
